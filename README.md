...
> repo **내부** 모듈이 형제 모듈의 검증된 부품을 재사용하는 그래프 (commons `@D g67`).
> intra-project only — 다른 repo와 잇지 않음. 부품 만들기 전 atlas + 형제 모듈 grep → 검증된 것 재사용.
> airgenome은 **단일-허브(hub-and-spoke)** 구조 — `airgenome/core/airgenome` 를 중심으로 모든 모듈이 방사형으로 의존
> (README 불변식 #2: "새 모듈은 `../../airgenome/core/airgenome` 만 import — 모듈 간 상호 import 금지").

### 1. 재사용 그래프

```
                       ┌──→ predictive_throttle/module
                       ├──→ filters/module/process · data  (5 직접 import)
airgenome/core/airgenome ──→ run.hexa
   (60-byte genome 허브)  ├──→ airgenome/core/test/core_test
                       └──→ tool/bench/* (28 모듈 총)
                                  │
tool/bench/bench_lib ──공통 helper──┘ (4 bench import)
                                  │ Target:
filters/module/process · data ←─verify pair─ tool/bench/bench_f{45,64,65,66}
```

### 2. 엣지 원장 (REAL · 코드 import / Target 명시로 입증)

| provides (출처) | primitive | reused by | 증거 (repo) |
|---|---|---|---|
| `airgenome/core/airgenome` | 60-byte genome core (Vitals · sample · assess · AdaptiveThrottle) | 28개 모듈 — predictive_throttle · filters ×5 · run · core_test · bench ×~21 | `use "…/airgenome/core/airgenome"` × 28 (불변식 #2) |
| `tool/bench/bench_lib` | 5B 단계 bench 공통 helper (ns 타이밍 · synth ring · 결과 보고) | `bench_f64` · `bench_f65` · `bench_f45` · `bench_f66` | `use "./bench_lib"` × 4 |
| `filters/module/process/safari_active_throttle_signal` | safari active-throttle 신호 필터 | `tool/bench/bench_f64_active_signal` | `// Target:` 명시 |
| `filters/module/data/safari_bg_tab_throttle_genome` | safari bg-tab throttle genome 필터 | `tool/bench/bench_f45_safari_genome` | `// Target:` 명시 |
| `filters/module/process/safari_youtube_gpu_filter` | safari youtube-gpu 필터 | `tool/bench/bench_f65_youtube_gpu` | `// Target:` 명시 |
| `filters/module/process/safari_battery_freeze_filter` | safari battery-freeze 필터 | `tool/bench/bench_f66_battery_freeze` | `// Target:` 명시 |

**REAL 엣지 = 6** (core 허브 1 + bench_lib 1 + bench↔filter verify-pair 4).

### 3. provides[] 레지스트리 (모듈별 제공 부품)

| 모듈 | 제공 primitive |
|---|---|
| `airgenome/core/airgenome` | 60-byte genome core · Vitals · sample · assess · AdaptiveThrottle (전 모듈 허브) |
| `tool/bench/bench_lib` | bench 공통 helper (ns diff · synth ring · 5-tuple 보고 표준) |
| `filters/module/data/columnar_projection` | JSONL 컬럼 투영 (jq full-scan 대비 2x~9x) — 반복 쿼리 캐시 패턴 *(candidate reused-by)* |
| `filters/module/data/prefix_trie_mmap` (PTBF) · `autocomplete_trie_mmap` (APBF) | prefix-trie / pool blob mmap 일반화 primitive *(candidate reused-by)* |
| `filters/module/transport/base` | hook 게이트 공통 utils (safe_exec · state 신선도) *(candidate reused-by)* |

### 4. 사용법 (atlas-first · @D g67)

1. 부품 만들기 전 — `hexa atlas lookup --prefix=<topic>` + 형제 모듈 grep (`grep -rl '<primitive>' --include="*.hexa"`)
2. 재사용 시 모듈 헤더 주석에 `// Target:` (bench) 또는 `use "…"` (import)로 엣지 스탬프
3. 이 §NEXUS 표에 엣지 1줄 추가 (그래프 + 원장 동기화)

### 5. 후보 엣지 (candidate · 미검증 — 코드 reuse 아직 없음)

| 후보 | 비고 |
|---|---|
| `columnar_projection` ↔ bench/forecast append 패턴 | 컬럼 캐시 패턴이 `genomes.ring`/`forecast.jsonl` 쿼리에 재사용 가능 — 미검증 |
| `prefix_trie_mmap`(PTBF) ↔ `autocomplete_trie_mmap`(APBF) | 동일 trie-blob 계열 — 공유 mmap 레이아웃 primitive로 통합 가능 — 미검증 |
| `filters/module/transport/base` 게이트 utils ↔ filters/data 모듈 | safe_exec · state-freshness 공통화 가능 — 현재 각자 인라인 — 미검증 |
| 나머지 ~77개 bench ↔ filters/data 필터 | bench↔filter 1:1 verify-pair는 4개만 `// Target:` 명시; 다수 bench는 synth-only (filter와 느슨 결합) — pair 미확정 |

> **단일-허브 노트**: airgenome의 실질 격자는 `core/` 1-허브 + bench harness이므로 cross-module 격자는 얕다 (의도된 불변식 #2 — 모듈 간 상호 import 금지). §5 후보는 향후 공통화 시 REAL로 승격.
