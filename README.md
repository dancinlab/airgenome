...
> 재사용 격자 SSOT → 루트 [`NEXUS.tape`](./NEXUS.tape) (6 REAL 엣지 + 4 candidate · 단일-허브 hub-and-spoke).
> intra-project only — 다른 repo와 잇지 않음. 부품 만들기 전 atlas + 형제 모듈 grep → 검증된 것 재사용 (commons `@D g67`).
