...
| 경로 | LoC | 역할 |
|---|---:|---|
| `airgenome/core/airgenome.hexa` | 395 | 메인 진입점 |
| `probe/module/probe.hexa` | 122 | Mac+remote vitals → infra_state.json |
| `dispatch/module/dispatch.hexa` | 319 | best host per workload |
| `harvest/module/harvest.hexa` | 388 | top-N processes → 60B hexagon |
| `label/module/label.hexa` | 355 | rule-match anomalies |
| `forecast/module/forecast.hexa` | 359 | Holt α/β smoothing 1h-ahead |
| `predictive_throttle/module/predictive_throttle.hexa` | 295 | 예측 throttle |
| `genome_merge/module/genome_merge.hexa` | 149 | genome merge |
| `exe_dispatch/module/exe_dispatch.hexa` | 191 | .exe → plugin forwarder |
...
### 1.3 predecessor SSOT 0 확인

- `archive/v1/cl_mk1` = single zsh script (Paul Falstad shell), legacy, **SSOT 0** (사용자 prompt 명시).
- `cl_mk1` 와 `mk2_hexa/` 는 사용자가 의도적으로 archive/v1/ 로 분리해 둔 predecessor → current transition 흔적.
...
4. **bench coverage gap**: 8 live modules 중 0개가 bench mode 보유 (wave5 캠페인은 filters/module/data/* 대상). triplet C 가 우선 권장.
...
7. **predecessor SSOT 0 해석**: 사용자 명시 "predecessor SSOT 0" 을 "legacy predecessor (cl_mk1 zsh) 은 진실 출처 아님" 으로 해석. 만약 다른 의미였다면 후속 정정.
...
- `archive/v1/cl_mk1` (predecessor SSOT 0 zsh script, READ-ONLY)
- `archive/v1/roadmap.md` (predecessor → current transition roadmap, "self-tuning thresholds" idea source)
- `airgenome/core/airgenome.hexa` (395 LoC entry)
