...
**대상**: `filters/module/process/{calendar,finder,mail,memo,safari,telegram}.hexa` 의 hot-path 6 site
...
own 10 new "airgenome-local: gate/filter ROI 직접 적용 사이트 명시 (process gate 6 filter hot-path 6 site, S1~S6)"
  ...
  exception filters/module/process/{compute,claude}.hexa 변경 면제 — compute = L0/AG6 frozen (own 9 exception 등록), claude = session_now.json 외부 의존 (own 9 2B 등록). claude 의 site-S4/S5 는 dependency declaration 후 재포함 가능. filters/module/data/* 및 filters/module/transport/* 변경 면제 (별도 stage own).
  bans filters/module/process/* 신규 코드에서 ps + awk + grep 3-fork chain 추가 — site-S2 native ps reader mandate
  bans filters/module/process/{calendar,mail,memo,safari,telegram}.hexa 신규 lsappinfo per-cycle fork 추가 (env-cache 미경유) — site-S3 cache mandate
  bans filters/module/process/claude.hexa 신규 ls/stat/date 3-fork chain 추가 — site-S4 A13 builtin 도래 후 mandate
  bans filters/module/process/claude.hexa 신규 session_now.json substring chain (9+ field 반복) — site-S5 A14 builtin 도래 후 mandate
  bans filters/module/process/{safari,telegram}.hexa 신규 O(NM) helper match 추가 (N>5 인 경우) — site-S6 Aho-Corasick mandate
  bans filters/module/process/* STATE_LOG 의 cycle 당 다중 append — site-S1 exception 정신 (이미 single-line 보장)
  ...
  follow-up filters/module/process/{calendar,finder,mail,memo,safari,telegram}.hexa 자체 self_test 또는 --mode=run-once 측정 자동화 (production exit=0 + state_*.jsonl byte-identical)
...
|---|---|---|---|---|
| own 5 | harvest (capture) | harvest/module/harvest.hexa | 5 (site-1~5) | DONE 4 / N/A 1 (site-4) |
| own 6 | forecast | forecast/module/forecast.hexa + genome_merge | 4 (site-6/7/8/8b) | DONE 3 / N/A 1 (site-7) |
| own 7 | label | label/module/label.hexa | 4 (site-9~12) | DONE 3 / N/A 1 (site-11) |
| own 8 | predictive_throttle | predictive_throttle/module/predictive_throttle.hexa | 2 (site-13/14) | DONE 2 |
| **own 10** | **process gate** | **filters/module/process/{6 filter}.hexa** | **6 (site-S1~S6)** | **DRAFT (S1 exception, S2~S6 등록 권장)** |
